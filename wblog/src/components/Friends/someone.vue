<template>
    <div class="someone">
            <div class="header">
                <div>
                    <img :src="str">
                </div>
                <div>
                    <img :src="arr">
                </div>
                <div>
                     <div class="back">
                    <router-link to="home">
                        <i class=" iconfont icon-zuojiantou1"></i>
                    </router-link>
                </div>
                <div class="amend">
                        <i class="iconfont icon-gengduo"></i>
                    </a>
                </div>
                </div>
                <div>
                    <h2>{{xx}}</h2>
                </div>
            </div>

            <div class="footer">
                    <div>
                    <h2>今天</h2>
                    <p class="pl">今天天气超棒</p>
                </div>
                <div>
                    <ul>
                        <li v-for="(item,index) in sz">
                            <img :src="item">
                        </li>
                    </ul>
                </div>
                <div>
                    <div>
                        <p><span>6</span>分钟前</p>
                    </div>
                    <div>
                          
                          <i class="iconfont icon-good">  
                          </i>  
                         
                          <router-link to="discuss">
                          <i class="iconfont  icon-message">
                              
                          </i>
                          </router-link>
                    </div>
                </div>
                <div>
                    <p>
                        <span>安琪</span>:
                        对呀天气好棒
                    </p>
                     <p>
                        <span>安琪</span>:
                        对呀天气好棒
                    </p>
                     <p>
                        <span>安琪</span>:
                        对呀天气好棒
                    </p>
                </div>
                
            </div>
            
        </div>   
            </div>
    </div>       
</template>
<script>
export default {
    data(){
        return{
            str:"../../../static/img/banner1.png",
            arr:"../../../static/img/touxiang.png",
             sz:[
                    "../../../static/img/u=3797481993,1929347741&fm=27&gp=01@3x.png",
                    "../../../static/img/u=3797481993,1929347741&fm=27&gp=01@3x.png",
                    "../../../static/img/u=3797481993,1929347741&fm=27&gp=01@3x.png",
                ],
                xx:"路西"
        }
    },
    
}
</script>
<style scoped>
    .header{
        width: 100%;
        height: 5.38rem;
       
    }
    .header>div:nth-child(1) img{
        width: 100%;
        height: 100%;
    }
    .header>div:nth-child(2){
        width: 1.2rem;
        height: 1.2rem;
        border-radius: 50%;
        background: #000;
        position: absolute;
        top: 3rem;
        left: 3rem;
    }
      .header>div:nth-child(2)>img{
          width: 100%;
          height: 100%;
      }
      .header>div:nth-child(3)>div:nth-child(1){
          position: absolute;
          top: .6rem;
          left: .18rem;
      }
      .header>div:nth-child(3)>div:nth-child(1) i{
          font-size: .5rem;
      }
       .header>div:nth-child(3)>div:nth-child(2) {
           position: absolute;
           top:.6rem;
           right:.6rem; 
       }
        .header>div:nth-child(3)>div:nth-child(2) i{
            font-size: .5rem;
        }
         .header>div:nth-child(4){
             font-size: .3rem;
            color: #000;
            position: absolute;
            left: 3.1rem;
            top: 4.6rem;
         }
        .footer{
            width: 100%;
            height: 6.2rem;
          
        }
          .footer div:nth-child(1){
          display: flex;
            padding-top: 0.2rem;
      }
       .footer div:nth-child(1) h2{
          margin-left: 0.47rem;
          font-size: .36rem;
      }
      .footer div:nth-child(1) .pl{
          color: #1D1D1D;
          font-size: 0.3rem;
           margin-top: 0.45rem;
          padding-left: 0.5rem;
      }
       .footer div:nth-child(2){
          display: flex;
          padding-top: 0.2rem;
          padding-left: 1.1rem;
      }
      .footer div:nth-child(2) ul{
          display: flex;
      }
      .footer div:nth-child(2) ul li img{
            width: 1.45rem;
            display: block;
            height: 1.45rem;
            margin: 0 .29rem;
        }
     .footer div:nth-child(3){
          display:flex;
      }
     .footer div:nth-child(3) div{
          padding-top: 0.3rem;
      }
       .footer div:nth-child(3) div p {
            padding-top: 0.2rem;
           color:#1D1D1D;
           padding-left: 1.5rem;
           font-size: 0.18rem;
           
       }
      .footer div:nth-child(3) div:nth-child(2){
           padding-left: 2.8rem;
       }
        .footer div:nth-child(3) div:nth-child(2) i:nth-child(1) {
           display: block;
           /* height: .31rem; */
            margin-left: 0.35rem;
             font-size: 0.4rem;
       }
        .footer div:nth-child(3) div:nth-child(2) i:nth-child(2) {
            /* width: 0.29rem;
            height: 0.29rem; */
            margin-left: 0.35rem;
            font-size: 0.5rem;
            display: block;
        }
        .footer div:nth-child(4){
            width: 4.93rem;
            height: 1.47rem;
            background: #F4F6F8;
            margin: 0 auto;
           margin-top: .29rem;
        }
        .footer div:nth-child(4) span{
            color: #0D70F8;
            font-size: .24rem;
             line-height: 0.48rem;
        }
        .footer div:nth-child(4) p{
            line-height: 0.48rem;
            color: #454545;
            padding-left: .2rem;
        }
</style>
